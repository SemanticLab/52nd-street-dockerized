<?
	...
	if (isset($_REQUEST['showTutorial'])){
		$isNoob = 'fs.showTutorial = true;';
	}
...
<head>
    ...
    <script type="text/javascript">
		...
		fs.storeRelationship = function(button){
			...
			$.post("/work/?action=storeRelationship", updateObj, function(data){					
		...
		fs.addBubble = function(idLocal){
 			...
 			$.get("/work/", { action: 'returnTextById', transcript: fs.transcriptMeta['md5'], id: idLocal },function(data){			
		...
		fs.buildBubbleDom = function(pair){
			...
			for (x in loopAry){
				...
				if (loopAry[x].type=='Q'){
					var useImage = "/img/no_image_square.png";
					var title = "Interviewer";
					var bubbleClass = 'question';					
					var imageHolderClass = 'questionImage';
				}else{
					var useImage = "/img/square/" + fs.intervieweeImage + '.png';
					var title = "";
		...
		fs.addComment = function(){
			...
			$.post("/work/?action=storeComment", postObj, function(data){					
		...
		fs.requestText = function(transcript, id){
			$.get("/work/", { action: 'returnTextById', transcript: fs.transcriptMeta['md5'], id: id },function(data){			
				return data;				
			});
		}
		...
		fs.requestMatches = function(){
			...
			$.get("/work/", { action: 'returnMatchesForURI', transcript: fs.transcriptMeta['md5'], uri: fs.activeURI },function(data){
				...
				var foundNull = false;
 				...
 				for (x in pairs){
					...
					if (typeof pairs[x].question == 'number'){	
						foundNull = true;					
						$.get("/work/", { action: 'returnTextById', transcript: fs.transcriptMeta['md5'], id: pairs[x].question },function(data){
					...
					if (typeof pairs[x].answer == 'number'){	
						foundNull = true;					
						$.get("/work/", { action: 'returnTextById', transcript: fs.transcriptMeta['md5'], id: pairs[x].answer },function(data){
		...
		$(document).ready(function($) {
			...
			$(".matchListItemLink").bind('click',function(event){			
				...
				if (useImage==''){
					useImage = "/img/no_image_square.png";	
					$("#scoreBoard img:last").css('opacity',0.5);	
				}else{
					useImage = "/img/square/" + useImage;
				...
				$("#scoreBoard img:first").attr('src',"/img/square/" + fs.intervieweeImage + '.png');
...
<body>
 ...
 -->
    ...
    <div class="container-fluid">
      ...
      <div class="row-fluid">
          ...
          <div id="logoHolder" rel="popover" data-value="skip" data-title="Back to Start Page" data-placement="bottom" data-html="true" data-trigger="manual" data-content="If you want to work on another musician or return to the start page just click the Linked Jazz logo. Don't worry you progress is continually saved, you can come back and start working where you left off at anytime.<br><br><button class='btn tutorialNext btn-primary'>Next</button>">
	          <a href="/" title="Return to homepage, select diffrent musican" alt="Return to homepage, select diffrent musican"><img src="/img/lj_52_logo.png"/></a>
        	...
        	<div class="row-fluid">
	    	  	...
	    	  	<div class="span8">        
                	<div id="scoreBoard">
                        ...
                        <div class="row-fluid">
                            <div class="span2">                         
                            	<img class="pull-left" src="/img/square/Charles_Davis_(saxophonist).png">
                            </div>
                            ...
                            <div class="span2">                         
                            <img class="pull-right" src="/img/square/Charles_Davis_(saxophonist).png">
                            </div>
						</div>                                                                                    
                    </div>
    	        </div>        	
	    	  	<div class="span2" style="position:relative; white-space: nowrap;">        
					...
					?>
                	<a href="?showTutorial=true" id="linkShowTutorial">Show Tutorial</a>
    ...
    >
    	<img src="/img/logo_small.png"></div>          
    </div>
   ...
   <div class="tutorial" id="tutorialFinishedAll" 
	  ...
	  rel="popover" data-title="Complete" data-placement="right" data-html="true" data-trigger="manual" data-content="Woah! You processed the entire transcript, way to go! <br><br> You have earned <span id='finalPoints'></span> points from this transcript.<br><br>You can <a href='/'>return to the start page</a> if you would like try another musican."
    >
    	<img src="/img/logo_small.png"></div>          
    </div>         
